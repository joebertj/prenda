 ...
 *
 */
 public class PawnHistory extends javax.servlet.http.HttpServlet implements javax.servlet.Servlet {
    /**
	 * 
	 */
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{
			pstmt = conn.prepareStatement("SELECT COUNT(pid) FROM pawn WHERE year(loan_date)=year(curdate())+? AND month(loan_date)=?");
			for(int i=1;i<=12;i++){
				pstmt.setInt(1,type);
				pstmt.setInt(2,i);
		...
		JFreeChart chart = ChartFactory.createTimeSeriesChart
		                     ("Pawn History",
		                      "Month",
		                      "Number of Pawned Items",
		                      dataset,
		                      true,
		                      true,
		                      true
		                     );
		ChartUtilities.saveChartAsJPEG(new File(getServletContext().getRealPath("/common/img")+"/pawnhistory"+type+".jpg"), chart, 500, 300);
