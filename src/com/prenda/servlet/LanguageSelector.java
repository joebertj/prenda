...
public class LanguageSelector {
	...
	private String set(HttpServletRequest request, HttpServletResponse response, ModelMap map, Locale locale,
		...
		/*int idxA = redirectUrl.indexOf("/prenda/admin/");
		int idxU = redirectUrl.indexOf("/prenda/customer/");
		int idxO = redirectUrl.indexOf("/prenda/owner/");
		int idxM = redirectUrl.indexOf("/prenda/manage/");
		int idxR = redirectUrl.indexOf("/prenda/register/");*/
		log.info("idx: " + idx);
		if(idx==0) { //prenda context
			...
			/*}else if(idxA==0){
				...
				redirectUrl = redirectUrl.substring(17);
			}else if(idxO==0){
				redirectUrl = redirectUrl.substring(14);
			}else if(idxM==0){
				redirectUrl = redirectUrl.substring(15);
			}else if(idxR==0){
				redirectUrl = redirectUrl.substring(17);*/
			}else{
