 ...
 *
 */
 public class PulloutHistory extends javax.servlet.http.HttpServlet implements javax.servlet.Servlet {
    /**
	 * 
	 */
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{
			pstmt = conn.prepareStatement("SELECT COUNT(pullout_date) FROM pullout WHERE year(pullout_date)=year(curdate())+? AND month(pullout_date)=?");
			for(int i=1;i<=12;i++){
				pstmt.setInt(1,type);
				pstmt.setInt(2,i);
		...
		JFreeChart chart = ChartFactory.createTimeSeriesChart
		                     ("Pullout History",
		                      "Month",
		                      "Number of Pulled-out Items",
		                      dataset,
		                      true,
		                      true,
		                      true
		                     );
		ChartUtilities.saveChartAsJPEG(new File(getServletContext().getRealPath("/common/img")+"/pullouthistory"+type+".jpg"), chart, 500, 300);
