...
<br/>
			Page 
<c:if test="${pagenum>1}">
				<A href='${contextPath}/admin/userlist.jsp?pagenum=<c:out value="${pagenum-1}"/>'>prev</A>
			</c:if>
<c:forEach var="i" begin="1" end="${pages}">
				<c:choose>
					<c:when test="${i!=pagenum}">
						<A href='${contextPath}/admin/userlist.jsp?pagenum=<c:out value="${i}"/>'><c:out
							value="${i}" /></A>
					</c:when>
					<c:otherwise>
						<c:out value="${i}" />
...
<c:if test="${pagenum<(pages-(adjust/perpage))}">
				<A href='${contextPath}/admin/userlist.jsp?pagenum=<c:out value="${pagenum+1}"/>'>next</A>
			</c:if>
			<TABLE border="1">
				<TR>
